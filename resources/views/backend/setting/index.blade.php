        ...
        <div class="card">
            ...
            <div class="card-datatable text-nowrap">
                <table class="table" id="datatable">
                    ...
                    <tbody>
                    @foreach($settings as $key => $user)
                        <tr>
                            <td>{{$loop->iteration}}</td>
                            <td>{{$user}}</td>
                            <td>
                                @can('permission-edit')
                                    <a href="{{route('setting.edit',$key)}}" class="btn btn-warning"><i class="fas fa-edit"></i></a>
                                @endcan
                                @can('permission-delete')
                                    <button type="button"  data-slug="{{ $key }}" class="btn btn-danger delete_button" data-bs-toggle="modal" data-bs-target="#exampleModal">
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <form action="{{ route('setting.destroy', 0) }}" method="POST" id="deleteForm">
                        @csrf
                        @method('DELETE')
...
<script>
    $(document).ready(function () {
        $(document).on("click", '.delete_button', function (e) {
            var id = $(this).data('id');
            var url = '{{ route("setting.destroy",":id") }}';
