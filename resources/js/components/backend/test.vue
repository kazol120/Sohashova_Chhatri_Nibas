          ...
          <div class="card-body">
            ...
            <div class="table-responsive" >
              <table class="table table-bordered table-hover align-middle text-center">
                ...
                <tbody v-if="!stockLoading && stockRows.length">
                  <tr v-for="(row, index) in stockRows" :key="row.product_id">
                    <td>{{ index + 1 }}</td>
                    <td class="fw-semibold">{{ row.product_name }}</td>
                    <td class="fw-bold text-info">{{ row.total_available_quantity }}</td>
                  </tr>
                </tbody>
                <tbody v-else>
                  <tr>
                    <td colspan="3" class="text-center py-4 text-muted">
                      <span v-if="stockLoading"><i class="fa fa-spinner fa-spin me-2"></i>Loading...</span>
...
export default {
  ...
  methods: {
    ...
    async loadProductStock() {
      ...
      try {
        const res = await axios.get(`${this.url}get-productstock`);
        this.stockRows = res.data.data || [];
      } catch (e) {
        this.toast("Failed to load product stock", "error");
      } finally {
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .print-header h2 { font-size: 18px; font-weight: bold; margin-bottom: 4px; }
    .print-header p { font-size: 12px; color: #666; }
    table { width: 100%; border-collapse: collapse; margin-top: 10px; }
    th, td { border: 1px solid #ccc; padding: 6px 8px; text-align: center; }
    thead th { background: #f59e0b !important; color: #fff !important; font-weight: bold; font-size: 11px; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
    ...
    .clickable-cell span { display: none !important; }
    .badge {
