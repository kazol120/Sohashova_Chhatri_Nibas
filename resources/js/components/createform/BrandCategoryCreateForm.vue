        ...
        <div class="xwrap">
          <div class="xbox" role="dialog">
            <div class="xhead d-flex justify-content-between align-items-center">
              <h5 class="mb-0">Create brand catgory</h5>
              <button type="button" class="btn btn-sm btn-light" @click="emitClose">
                <i class="fa fa-times"></i>
              </button>
            </div>
            <form @submit.prevent="createbarndcategory">
              <div class="xbody">
                <div class="mb-3">
                  <label class="form-label fw-semibold">
                    brand category Name <code class="req">*</code>
                  </label>
                  <input
                    type="text"
                    class="form-control"
...
export default {
  ...
  methods:{
    ...
    validate(){
      ...
      if(!this.form.name){
        this.errors.name = "brand category name is required"
      }
  ...
  try {
    ...
    const res = await axios.post(
      this.url + "brand-category-create",
      {
        name: this.form.name
      }
    )

    this.toast(res.data.message || "Brand Created")
    this.$emit("created")
    this.emitClose()
    ...
    this.toast(data?.message || "Create Categoryfailed", "error")
