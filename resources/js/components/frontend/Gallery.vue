      ...
      <div class="col-12 col-lg-10 col-xl-12">
        <div class="card mb-4 shadow-sm">
          <div class="card-header d-flex flex-wrap gap-2 justify-content-between align-items-center py-3">
            <div>
              <h5 class="card-title mb-0">Your Gallery</h5>
              <small class="text-muted">Manage gallery images displayed on homepage</small>
            </div>
            <button class="btn btn-primary" type="button" @click="openCreateFromComponent">
              <i class="ti ti-plus me-1"></i> Add Gallery Image
            </button>
          </div>
            ...
            <div class="table-responsive">
              <table class="table table-bordered table-hover align-middle">
                ...
                <tbody v-if="images.length">
                  <tr v-for="(img, idx) in images" :key="img.id">
                    <td>{{ from + idx }}</td>
                    <td>
                      <img v-if="img.image_url" :src="img.image_url" class="img-thumb" alt="gallery image" />
                      <span v-else class="text-muted small">No image</span>
                    </td>
                    <td>
                      <div class="d-flex gap-2">
                        <button class="btn btn-sm btn-primary" @click="openEditModal(img)">
                          <i class="ti ti-edit"></i>
                        </button>
                        <button class="btn btn-sm btn-danger" @click="openDeleteModal(img)">
                ...
                <tbody v-else>
                  <tr>
                    <td colspan="3" class="text-center py-4 text-muted">
                      ...
                      <span v-else>No gallery images found</span>
                    </td>
                  </tr>
                </tbody>
              </table>
                  ...
                  <div class="row g-3">
                    <div class="col-12">
                      <label class="form-label">
                        Image <small class="text-muted">(optional)</small>
                      </label>
                      ...
                      <div class="mt-2" v-if="edit.preview || edit.form.image_url">
                        <div class="preview-box">
                          <img :src="edit.preview || edit.form.image_url" alt="preview" />
                        </div>
        ...
        <div v-if="del.open" class="xmask" @click.self="closeDelete">
          <div class="xwrap">
            <div class="xbox xsmall" role="dialog" aria-modal="true">
              <div class="xhead d-flex justify-content-between align-items-center">
                <h5 class="mb-0 text-danger">Delete Gallery Image</h5>
              ...
              <div class="xbody">
                <div class="alert alert-warning mb-0">
                  Are you sure you want to delete this gallery image (ID: <b>{{ del.item?.id }}</b>) ?
                </div>
              </div>
...
import GalleryCreate from "../../components/createform/GalleryCreate.vue";

export default {
  name: "GalleryList",
  components: { GalleryCreate },
  ...
  data() {
    return {
      ...
      edit: {
        open: false,
        form: { id: null, image: null, image_url: "" },
        preview: "",
      },
  ...
  watch: {
    search() {
      ...
      this._t = setTimeout(() => this.fetchImages(1), 300);
    },
    perPage() {
      this.fetchImages(1);
    },
  ...
  methods: {
    ...
    async fetchImages(page = 1) {
      this.loading = true;
      try {
        const res = await axios.get(`${this.url}gallery-get`, {
        ...
        console.error(e);
        this.toast("Failed to load gallery images", "error");
      } finally {
        this.loading = false;
      }
    },
    ...
    async handleCreated() {
      ...
      await this.fetchImages(this.currentPage);
    },
    ...
    async updateImage() {
      ...
      try {
        ...
        const res = await axios.post(
          `${this.url}gallery-update/${this.edit.form.id}`,
          fd,
    ...
    async confirmDelete() {
      ...
      try {
        const res = await axios.delete(`${this.url}gallery-delete/${this.del.item.id}`);
        this.toast(res.data?.message || "Deleted ✅", "success");
        this.closeDelete();
        const willBeEmpty = this.images.length === 1 && this.currentPage > 1;
...
.preview-box { width: 100%; max-width: 590px; border: 1px solid #e5e7eb; border-radius: 12px; overflow: hidden; background: #fff; }
